...
double HomographyModel::Error(const Mat &H, size_t index) const {
  libNumerics::vector<double> x1(3);
  x1(0) = x1_(0,index); x1(1) = x1_(1,index); x1(2) = 1.0;
  x1 = H * x1;
  if(x1(2)<=0)
    return std::numeric_limits<double>::infinity();
  x1 /= x1(2);
  return (sqr(x2_(0,index)-x1(0)) +
          sqr(x2_(1,index)-x1(1)));
}
